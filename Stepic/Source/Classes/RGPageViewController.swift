...
open class RGPageViewController: UIViewController {
  ...
  private func updatePager(_ index: Int) {
    ...
    if index == currentPageIndex {
      ...
      pager.setViewControllers([viewController], direction: direction, animated: true, completion: { [weak self] (_) -> Void in
        self?.animatingToTab = false
      ...
      pager.setViewControllers([viewController], direction: index < currentPageIndex ? .reverse : .forward, animated: true, completion: { [weak self] (_) -> Void in
        self?.animatingToTab = false
      })
    }
