...
protocol DownloadingServiceProtocol: AnyObject {
    ...
    /// Download data by URL and once a download request has successfully completed moves the temporary file to it
    /// final location based on provided filename.
    func download(url: URL, destination destinationFilename: String) throws -> DownloaderTaskProtocol.IDType
...
final class DownloadingService: DownloadingServiceProtocol {
    ...
    func download(url: URL, destination destinationFilename: String) throws -> DownloaderTaskProtocol.IDType {
        if self.taskIDByDownloadURL[url] != nil {
            throw Error.alreadyDownloading
        }

        let task = DownloaderTask(url: url)
        self.setupReporters(for: task, withDestination: destinationFilename)
        task.start(with: self.downloader)

        self.taskIDByDownloadURL[url] = task.id
    ...
    private func setupReporters(for task: DownloaderTaskProtocol, withDestination destinationFileName: String) {
        let taskID = task.id
    ...
    private func removeObservedTask(id: DownloaderTaskProtocol.IDType) {
        ...
        if let taskURL = self.taskIDByDownloadURL.first(where: { $1 == id })?.key {
            self.taskIDByDownloadURL.removeValue(forKey: taskURL)
        }
    }
